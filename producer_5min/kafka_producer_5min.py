...
API_KEY = os.getenv("ALPHAVANTAGE_API_KEY_5min")
...
# Validate API Key
if not API_KEY:
    logger.error("ALPHAVANTAGE_API_KEY_5min environment variable is not set.")
    exit(1)
    ...
    for symbol in SYMBOLS:
        try:
            ...
            if not meta_data and not time_series:
                logger.error(f"Meta Data or Time Series (5min) is missing for symbol {symbol}.")
                continue

            last_refreshed = datetime.fromisoformat(meta_data.get("3. Last Refreshed"))
            last_update = get_last_update(symbol) or (last_refreshed - timedelta(days=1))

            for timestamp, value in time_series.items():
                timestamp_dt = datetime.fromisoformat(timestamp)
                if not last_update or timestamp_dt > last_update:
                    message = {"symbol": symbol, "timestamp": timestamp, "data": {timestamp: value}}
                    producer.send('stock_prices_5min', message)
                    logger.info(f"Sent data for symbol: {symbol}, timestamp: {timestamp}")
...
try:
    ...
    producer.send('stock_prices_5min', {'terminate': True})
